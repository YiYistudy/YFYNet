...
EXIT_FAILURE = 1
inputFolderPath = "."
sourceScriptPath = "RunYeNet.py"
scriptName = "RunYeNet.py"
judgeName = "YeNetmodel.pth"
commentSymbols = ["#", "@", "%", "//", "__"]
...
def main():
	for f in os.listdir(inputFolderPath):
		...
		if os.path.isdir(target) and skipIfContains.lower() not in f.lower():
	...
	for f in os.listdir(inputFolderPath):
		target = os.path.join(inputFolderPath, f)
		if os.path.isdir(target) and skipIfContains.lower() not in f.lower() and not isCommented(f):
			scriptPath = os.path.join(target, scriptName)
